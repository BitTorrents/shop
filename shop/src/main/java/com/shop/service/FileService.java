...
public class FileService {
    ...
    public String uploadFile(String uploadPath, String originalFileName, byte[] fileData) throws Exception{
        ...
        // 바이트 단위의 출력을 내보내는 클래스(생성자로 파일이 저장될 위치와 파일의 이름을 넘겨 파일에 쓸 파일 출력 스트림을 만듭니다
    ...
    public void deleteFile(String filePath) throws Exception{
        File deleteFile = new File(filePath); // 파일지 저장된 경로를 이용하여 파일 객체를 생성
        if(deleteFile.exists()) { // 해당 파일이 존해하면 파일을 삭제한다
            deleteFile.delete();
